...
struct one_test_data {
    const char *in;
    int res;
    struct { const char *addr; const char *iface; const char *ns; const char *port; } expected;
};
...
static struct one_test_data test_data[] = {
    ...
    { "[hostname]@if:833", 1, { "hostname", "if", "",   "833"  } },
    { "hostname@@ns:42",   1, { "hostname", "",   "ns", "42"   } },
    { "hostname@if@ns:42", 1, { "hostname", "if", "ns", "42"   } },
    { "hostname@@:42",     1, { "hostname", "",   "",   "42"   } },
    { "[hostname]?",       0, {                                } },
    { "[hostname",         0, {                                } },
    { "hostname@@:",       0, {                                } },
    { "@if:844",           1, { "",         "if", "",   "844"  } },
    ...
    { "[::1]@if:2",        1, { "::1",      "if", "",   "2"    } },
    { "[::1]@@ns:2",       1, { "::1",      "",   "ns", "2"    } },
    { "[::1]@if@ns:2",     1, { "::1",      "if", "ns", "2"    } },
    { "[@bogus]@if@ns:2",  1, { "@bogus",   "if", "ns", "2"    } },
    { "::1@if@ns:2",       1, { "::1",      "if", "ns", "2"    } },
    { "[::1]:2@if",        0, {                                } },
};
        ...
        if (res && p->res) {
            ...
            OKF(strcmp(ep_str.iface, p->expected.iface) == 0,
                ("%s: network interface %s <> %s", p->in, ep_str.iface,
                 p->expected.iface));
            OKF(strcmp(ep_str.ns, p->expected.ns) == 0,
                ("%s: network namespace %s <> %s", p->in, ep_str.ns,
                 p->expected.ns));
